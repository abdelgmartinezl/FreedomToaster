...
t = os.system("du -h -m /home/potty/Toquin/os/"+x+".iso | cut -c 1-4")
...
class USB(clutter.Texture):
	...
	def clicked(self,stage, event):
		if lusb > t:
			os.system("dd if=/home/potty/Toquin/os/"+x+".iso of=/dev/sdb bs=8M")
			exit()
		else:
			print " - NO SE PUEDE - "
			exit()
...
class CD(clutter.Texture):
	...
	def clicked(self,stage, event):
		if lcd > t:
			os.system("cdrecord -eject /home/potty/Toquin/os/"+x+".iso")
			exit()
		else:
			print " - NO SE PUEDE - "
			exit()
